...
def neigbours(x,vectDep,Matrix):    '''Fuction to get the corresponding neighbour list from the neighbours matrix'''
    neigh=[]
    for y in Matrix[x]:
        if (Matrix[x][y]==1):
            neigh.append(y)
    return neigh
def SetNeighbours(Matrix): '''Creates the matrix vector with the n-tuples and the neighbors matrix''' '''À modifier selon préferences'''
    Matrixin= []
...
custom_style = Style( '''Fuction to set the colors in red'''
